...
public class SettingsPanelBehaviourMB : MonoBehaviour
{
    ...
    private EcsPool<TimeScaleEvent> _timeScalePool;
    ...
    public void Init(EcsWorld world, GameState state)
    {
        ...
        _timeScalePool = _world.GetPool<TimeScaleEvent>();
        _vibrationEvent = _world.GetPool<VibrationEvent>();

        GetOnStartButtonSprite();
    }
    #endregion
        ...
        public void MoveSettingsPanel()
        {
            ...
            if(!_wasClicked)
            {
                ...
                _wasClicked = true;
                _timeScalePool.Add(_world.NewEntity()).TimeScale = 0;
            }
            else
            {
                _thisPanel.DOAnchorPosY(_startPos.y, _speed, false).SetUpdate(true);
                _wasClicked = false;
                _timeScalePool.Add(_world.NewEntity()).TimeScale = 1;
        }
}
    ...
    public void ChangeVibration()
        ...
        private void GetOnStartButtonSprite()
        {
            _vibroButton.sprite = GetVibroButtonSprite(_state.Vibration);
            _soundsButton.sprite = GetSoundsButtonSprite(_state.Sounds);
            _musicButton.sprite = GetMusicButtonSprite(_state.Music);
